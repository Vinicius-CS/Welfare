...
public class NOrientações extends JInternalFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NOrientações frame = new NOrientações();
	...
	public NOrientações() {
		...
		btnEditar.setBounds(46, 292, 66, 65);
		...
		btnExcluir.setBounds(46, 368, 66, 65);
	...
	private void consultar(){
        ...
        String sql = "select ID, Nome, EMail, Orientação from orientação where IDADM = ? and Nome like ?";
	...
	private void alterar(){
        String sql = "update orientação set Nome = ?, EMail = ?, Orientação = ?, IDADM = ? where ID = ?";
        try {
            ...
            if(adicionado > 0){
                JOptionPane.showMessageDialog(null, "A orientação foi alterada!");
	...
	private void remover(){
        ...
        if (confirmar == JOptionPane.YES_OPTION){
            String sql = "delete from orientação where EMail = ?";
            try {
                pst = conexao.prepareStatement(sql);
            ...
            if(apagado > 0){
                JOptionPane.showMessageDialog(null, "A orientação foi apagada!");
                limparCampos();
