...
import TelasPaciente.PConfigurações;
...
public class NConfigurações extends JInternalFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					NConfigurações frame = new NConfigurações();
...
private void consultar(){
        ...
        String sql = "select ID, Nome, Telefone, EMail, Senha from adm where EMail = ?";
        
        try {
            NLogin login = new NLogin();
            pst = conexao.prepareStatement(sql);
            pst.setString(1, login.getEMAILNUTRI());
            rs = pst.executeQuery();
        ...
        String sql = "update adm set Nome = ?, Telefone = ?, Senha = ? where ID = ?";
        try {
            NLogin login = new NLogin();
            pst = conexao.prepareStatement(sql);
